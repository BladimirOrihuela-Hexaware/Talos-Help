...
describe("Licensing microservice (e2e)", () => {
    ...
    beforeAll(async () => {
        ...
        app = moduleFixture.createNestApplication<NestExpressApplication>().useBodyParser("text");
        await app.init();
        httpServer = app.getHttpServer();

        talosPrivateKey = await openpgp.readPrivateKey({
            ...
            =9oeq
            -----END PGP PRIVATE KEY BLOCK-----`.replace(/^ +/gm, '')
        });
    });
    ...
    describe("Desktop requests", () => {
        ...
        `("should $name", async ({_, expectedStatusCode, mode}) => {
            const body = {
                ts: Date.now(),
                licenseId: "aaaaa", // TODO change these values
                machineId: "bbbbb"
            };
            const signedMsg = await openpgp.sign({
